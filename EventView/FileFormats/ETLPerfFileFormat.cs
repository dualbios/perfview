...
namespace EventView.FileFormats
{
    public class ETLPerfFileFormat : IFileFormat
    {
        ...
        private List<IFilePart> m_Children;
        ...
        public TraceLog GetTraceLog(string filePath, CommandLineArgs commandLineArgs,  TextWriter log, Action<bool, int, int> onLostEvents = null)
        {
            ...
            {
                etlxFile = Utilities.CacheFiles.FindFile(dataFileName, ".etlx");
            ...
            catch (Exception)
            {
                if (cachedEtlxFile)
                {
                    ...
                    Microsoft.Diagnostics.Utilities.FileUtilities.ForceDelete(etlxFile);
                    if (!File.Exists(etlxFile))
                    {
